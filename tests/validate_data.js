...
describe('YAML Validator', function() {
	describe('scams.yaml', function() {
		...
		it('every entry should have valid keys (id/name/coin/url/category/subcategory/description/addresses)', function() {
			assert.deepEqual(yaml.safeLoad(fs.readFileSync('./_data/scams.yaml', 'utf8')).filter(entry => Object.keys(entry).some(key => !['null','id','name','coin','url','category','subcategory','description','addresses'].includes(key))),[]);
		});
	});
