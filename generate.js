...
/* --clean function */
function clean() {
    var files = ["index.html", "_data/scams_compiled.json", "_data/addresses_compiled.json", "_data/ips_compiled.json", "_data/search_compiled.json"];
    var folders = ["address", "faq", "ip", "redirect", "report", "scam", "scams", "search"];
...
function yaml2json() {
    ...
    fs.readFile("./_data/archive_compiled.json", function(err, archive) {
		...
		fs.writeFile("./_data/search_compiled.json", JSON.stringify(search), function(err) {
...
function generatestatic() {
    ...
    for (var key = 0, len = scams.length; key < len; key++) {
        ...
        if (!fs.existsSync("./scam/" + scams[key]['id'] + "/")) {
            fs.mkdirSync("./scam/" + scams[key]['id'] + "/");
        }
        fs.writeFile("./scam/" + scams[key]['id'] + "/index.html", layout, function(err) {
            if (err) {
                return console.log(err);
            }
        });
    ...
    Object.keys(addresses).forEach(function(key) {
        ...
        if (!fs.existsSync("./address/" + key + "/")) {
            fs.mkdirSync("./address/" + key + "/");
        }
        fs.writeFile("./address/" + key + "/index.html", layout, function(err) {
            if (err) {
                return console.log(err);
            }
        });
    ...
    Object.keys(ips).forEach(function(key) {
        ...
        if (!fs.existsSync("./ip/" + key + "/")) {
            fs.mkdirSync("./ip/" + key + "/");
        }
        fs.writeFile("./ip/" + key + "/index.html", layout, function(err) {
            if (err) {
                return console.log(err);
            }
        });
                        ...
                        if (total_2 == scams.length) {
                            pages.forEach(function(page, index) {
                                ...
                                if (index == 0) {
                                    ...
                                    fs.writeFile("./scams/index.html", layout, function(err) {
                                        if (err) {
                                            return console.log(err);
                                        }
                                    });
                                ...
                                layout = layout.replace(/{{ scams.pagination }}/ig, pagination);
                                if (!fs.existsSync("./scams/" + (index + 1) + "/")) {
                                    fs.mkdirSync("./scams/" + (index + 1) + "/");
                                }
                                fs.writeFile("./scams/" + (index + 1) + "/index.html", layout, function(err) {
                                    if (err) {
                                        return console.log(err);
                                    }
                                });
        ...
        files.forEach(function(file) {
            if (file != "address.html" && file != "default.html" && file != "scam.html" && file != "ip.html" && file != "scams.html" && file != "scams_2.html") {
                fs.readFile('./_layouts/' + file, 'utf8', function(err, data) {
                    ...
                    if (file != "index.html") {
                        if (!fs.existsSync("./" + file.replace('.html', '') + "/")) {
                            fs.mkdirSync("./" + file.replace('.html', '') + "/");
                        }
                        var filename = "./" + file.replace('.html', '') + "/index.html";
                    } else {
